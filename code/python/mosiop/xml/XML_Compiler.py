...
class XMLCompiler(object):
    ...
    # For compiling Modelica-models to XML-format.
    
    def __init__(self, libpaths=[], model_paths=[]):
        """ Compile to XML. """
        
        if not "USER" in os.environ.keys():
            os.environ["USER"] = "om"
        ...
        for path in libpaths:
            self.load_file(path)
        # self.load_lib("/home/marius/modelica-buildings/Buildings/package.mo")
        for path in model_paths:
             self.load_file(path)

    def load_MSL(self):
        ''' 
        Load the Modelica standard library (which? where?) 
        '''
        # must also be able to load MSL 4.0.0
        self.omc.sendExpression('loadModel(Modelica)')

    def load_file(self, path):
        om_cmd = 'loadFile("' + path + '")'
        return self.omc.sendExpression(om_cmd)

    def translate_to_xml(self, modelname, error_str=True):
        """ Model must be loaded. """
        xml_path = self.omc.sendExpression(f'translateModelXML({modelname})')
        # should go to logger:
        if error_str:
            print(self.omc.sendExpression('getErrorString()'))

        #patched_xml_path = xml_path.rstrip(".xml") + "_patched.xml"
        #self.patch_xml(xml_path, patched_xml_path)
        return xml_path
...
#xml_comp = XMLCompiler(libpaths=["/home/marius/modelica-buildings/Buildings/package.mo"], \
#                                #"/home/marius/bestest_air/models/BESTESTAir/package.mo"], \
#                      model_paths=["/home/marius/Test.mo"])

#path = xml_comp.translate_to_xml("Test")

#path = "TestCase5.xml"

#xml_comp.patch_xml(path, "test.xml")

#inspect_xml("test.xml")
