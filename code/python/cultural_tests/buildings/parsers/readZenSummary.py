...
filename='../../Data/Statkraft/ZENcsv/AllFiles/buildingsummary.csv'
dfsummary=pd.read_csv(filename, sep=';', index_col=0)
...
def plotbTIdToutBetavsSpesHeat(bid, name=False):
    df=getbTIdSummary(bid)
    fig,ax = plt.subplots(1,1)
    ax.scatter(x=df['sumHeat']/df['Area'], y=df['Tout_beta'])
    if name: #names each point with identifier
        for i, txt in enumerate(df['bEnum'].values):
            ax.annotate(txt, ((df['sumHeat']/df['Area']).values[i],(df['sumHeat']/df['peakHeat']).values[i]))
    ax.set_ylabel('Tout beta [Wh/m2K]')
    ax.set_xlabel('Spesific Heat [kWh/m2]')
    ax.set_title(bid)
    return