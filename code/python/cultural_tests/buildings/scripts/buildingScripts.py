...
#set path data
directory = '../../Data/Phd Lindberg/ZENcsv/AllFiles/'
...
'''
Create buildings from ZEN csv data files
'''
def createBuildingFromZENcsvData(fname):
    #creates building from single file
    name=fname.split('.')[0]
...
def createBuildingsFromZENcsvFolder(directory):
    #creates dictionary of bulidings from folder with ZEN csv files
    #directory specifies the folder
    Mdict, Bdict=zen.readAllcsvsFromFolder(directory)
    buildDict={}
    for key in Mdict:
        building=bd.building(name=key)
        building.zenMetaData=Mdict[key]
        if 'HtTot' in Bdict[key].columns: 
            ...
            print('Warning: ' + key + 'No HtTot')
        if 'HtSpace' in Bdict[key].columns:
            building.yearlyHtSpace=Bdict[key].HtSpace.groupby([Bdict[key].index.month, Bdict[key].index.day, Bdict[key].index.hour]).mean().mean()*365*24
        else:
            print('Warning:'+key+' HtSpace not available')
        if 'HtDHW' in Bdict[key].columns:
            building.yearlyHtDHW=Bdict[key].HtDHW.groupby([Bdict[key].index.month, Bdict[key].index.day, Bdict[key].index.hour]).mean().mean()*365*24
        else:
...
def createBulidingSummarydf(bDict):
    #creates a dictionary with summary of all buildings
    sumLst=[]
    for key in bDict:
        sumLst.append(bDict[key].getSummaryDict())
    df=pd.DataFrame(sumLst, columns=list(sumLst[0].keys()))
    return df



'''
Create regression files for regression in R
'''
def createRegFilesFromZenBdict(d, endpath, season=None, day=None, normalize=False, resample=None):
    #create regression files from a dictonary of zen data series
    #enpath = path to store the files
    #season = extract data from single season (Winter, Summer or Middle). None for all data
    #day = weekday or weekend (wd or we). None for all days
    #normalize = normalize the profile based on yearly heat consumption
    #resample = frequency of resample. None for no resample
    for key in d:
        df=d[key].copy()
        if normalize:
            df.HtTot=df.HtTot.groupby([df.index.month, df.index.day, df.data_reg.index.hour]).mean().mean()*365*24
        createRegFileFromDf(df, endpath, key, season=season, day=day, resample=resample)
    return


def createRegFilesFromBuildingDict(d, endpath, season=None, day=None, normalize=False, spesific=False, resample=None):
    #create regression files from a dictonary of building classes
    #enpath = path to store the files
    #season = extract data from single season (Winter, Summer or Middle). None for all data
    #day = weekday or weekend (wd or we). None for all days
    #normalize = normalize the profile based on yearly heat consumption
    #spescific = divide consumption on heated area
    #resample = frequency of resample. None for no resample
    for key in d:
        createRegFilesFromBuilding(d[key], endpath, season=season, day=day, normalize=normalize, spesific=spesific, resample=resample)
    return

def createRegFilesFromBuilding(b, endpath, season=None, day=None, normalize=False, spesific=False, resample=None):
    #create regression files from a building class
    #enpath = path to store the files
    #season = extract data from single season (Winter, Summer or Middle). None for all data
    #day = weekday or weekend (wd or we). None for all days
    #normalize = normalize the profile based on yearly heat consumption
    ...
    df=b.data_reg.copy()
    if spesific:
        if 'Floor_area_heat' in b.zenMetaData:
            div=float(b.zenMetaData['Floor_area_heat'])
        else:
            div=float(b.zenMetaData['Floor_area_total'])
        if not div > 0:
            div=1000
            print('Warning: ' + b.name + ' area not available')
        df.HtTot=df.HtTot/div*1000
    if normalize:
        df.HtTot=df.HtTot/b.yearlyHeatConsumption
    createRegFileFromDf(df, endpath, b.name, season=season, day=day, resample=resample)
    return


def createRegFileFromDf(df, endpath, name, season=None, day=None, resample=None):
    #create regression files from a dataframe 
    #enpath = path to store the files
    ...
    #resample = frequency of resample. None for no resample
    try:
        if resample: df=bd.resampleBuildingData(df, resample)
    except:
        print ('Warning: resampling failed. csv not created')
        return
